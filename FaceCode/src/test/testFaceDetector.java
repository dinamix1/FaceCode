...
public class testFaceDetector {
	...
	public static void main(String[] args) throws IOException {
		...
		//ImageIO.write(res,"png",new File("Pictures//training_dataset_out//test3.png"));
		...
		Mat image = Highgui.imread("Pictures//training_dataset_png//" + filename);
		...
		if(eyeDetections.toArray().length == 1) {
			for (Rect rect : eyeDetections.toArray()) {
				...
				int newwidth = (int)Math.ceil(1.9 * (3.2/2.43) * rect.width);
				...
				BufferedImage crop = ImageUtils.reSize(originalimage.getSubimage(newXleft,newYtop,newwidth,newheight),250,190);
				ImageIO.write(crop,"png",new File("Pictures//training_dataset_out//" + filename));
			}
		}
		else {
			BufferedImage resi = ImageUtils.reSize(originalimage,250,190);
			ImageIO.write(resi, "png", new File("Pictures//training_dataset_out//" + filename));
		}
